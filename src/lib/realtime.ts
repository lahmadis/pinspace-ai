...
async function findOrCreateCritSession(
  sessionId: string,
  boardId: string
): Promise<{ id: string; session_id: string; board_id: string; status: string } | null> {
  // ========================================================================
  ...
  try {
    ...
    // ========================================================================
    const createPayload = {
      boardId: boardId, // ✅ Valid UUID (validated above)
      sessionId: sessionId, // ✅ Valid UUID (validated above)
      status: 'active',
    ...
    console.log('[realtime] Response data:', createSessionResponseData.session ? 'Present' : 'Missing');
    if (createSessionResponseData.session) {
      console.log('[realtime] Created session:', {
        id: createSessionResponseData.session.id,
        session_id: createSessionResponseData.session.session_id,
        board_id: createSessionResponseData.session.board_id,
        status: createSessionResponseData.session.status,
      });
      console.log('[realtime] ✅ Created new crit session:', createSessionResponseData.session.id);
      return createSessionResponseData.session;
    }
...
export async function createGuestWithRetry(
  ...
  try {
    // ========================================================================
    // Step 1: Fetch existing crit session (guests can only join, never create)
    // ========================================================================
    // Guests should only join existing sessions created by the host.
    // If the session doesn't exist, we return null (guests cannot create sessions).
    // If it exists but is inactive, we return null (guests shouldn't join inactive sessions).
    // REFACTORED: Use sessionId (UUID) directly, no sanitization needed
    // ========================================================================
    console.log('[realtime] 📡 Fetching existing session for guest:', sessionId);
    const getSessionResponse = await fetch(`/api/crit-sessions/${sessionId}`);
    
    if (!getSessionResponse.ok) {
      console.error('[realtime] ❌ Session not found for guest:', sessionId);
      return null;
    }

    const getSessionResponseData = await getSessionResponse.json();
    const session = getSessionResponseData.session || getSessionResponseData.data || getSessionResponseData;
